...
# ── Reports ────────────────────────────────
    ...
    # ────────── ENTERPRISE AGGREGATION FILTER ──────────
    vehicle_registry = {}

    for record in engine.anpr.records:
        tid = record.get('track_id')
        plate_text = str(record.get('plate', '')).strip().upper()
        
        # Clean out obvious low-confidence noise artifacts
        if len(plate_text) <= 1 or any(char in plate_text for char in ["?", "!", "#"]):
            continue
            
        if tid not in vehicle_registry:
            vehicle_registry[tid] = {
                "track_id": tid,
                "best_plate_guess": plate_text,
                "all_historical_guesses": [plate_text],
                "last_seen_timestamp": record.get('timestamp')
            }
        else:
            vehicle_registry[tid]["all_historical_guesses"].append(plate_text)
            # Enterprise Logic: Keep the longest, most clear alphanumeric sequence
            if len(plate_text) > len(vehicle_registry[tid]["best_plate_guess"]):
                vehicle_registry[tid]["best_plate_guess"] = plate_text
            vehicle_registry[tid]["last_seen_timestamp"] = record.get('timestamp')

    # Convert our clean key-value map into a clean list for the frontend
    final_anpr_data = list(vehicle_registry.values())
    
    # Save the consolidated file for your frontend team to fetch
    import json
    with open(reports / 'anpr_summary.json', 'w') as f:
        json.dump(final_anpr_data, f, indent=4)
